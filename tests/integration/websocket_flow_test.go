...
func TestClientReceivesCrowdingAwareAutonomousSteering(t *testing.T) {
	...
	for {
		...
		if snapshot.AutonomousCircles[0].X >= initial.AutonomousCircles[0].X {
			t.Fatalf("expected autonomous circle to steer away from more crowded direction, before=%v after=%v", initial.AutonomousCircles[0].X, snapshot.AutonomousCircles[0].X)
		}
		return
	}
}
